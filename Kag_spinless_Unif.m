%% runing parameters
runtype=0; %% 1 for cluster run and 0 for local run 
...
% fprintf('Start spinless TUfRG flow with mu=%.2f, U=%.2f, V1=%.2f, V2=%.2f, Nf=%d, Lq=%.0f, Lk=%.0f!\n',mu,U,V1,V2,Nf,Lq,Lk)
fprintf('Start spinless TUfRG flow with mu=%.2f, U=%.2f, V1=%.2f, V2=%.2f, V3=%.2f, Nf=%d, Lq=%.0f, Lk=%.0f!\n',mu,U,V1,V2,V3,Nf,Lq,Lk)
if (parallelrun==1 )
    if (runtype ==1)
        c = parcluster('local');
        c.NumWorkers = Ncores;
...
%     fprintf('Iter=%d, Lambda=%.16f, dLambda=%.16f, Pmax=%.16f, Cmax=%.16f, Dmax=%.16f\n',iter,lambda,dlambda,vmaxp,vmaxc,vmaxd);
...
%     vmax=max([vmaxp,vmaxc,vmaxd,vmaxk]);
    if vmaxp>vstop
        fprintf("P INSTABILITY!!!\n")
    elseif (vmaxc>vstop || vmaxd>vstop )
        fprintf("K INSTABILITY!!!\n")
%     elseif vmaxd>vstop
%         fprintf("K INSTABILITY!!!\n")
    elseif lambda<lambdamin
...
% % % str=['Kagome_spinless_Tflow_',num2str(Nf),'FFs_qmesh=',num2str(Lq),'_kmesh=',num2str(Lk),'_mu=',num2str(mu),'_U=',num2str(U),'_V1=',num2str(V1),'_V2=',num2str(V2),'.mat'];

% str=['Kagome_spinless_Tflow_',num2str(Nf),'FFs_qmesh=',num2str(Lq),'_kmesh=',num2str(Lk),'_mu=',num2str(mu),'_U=',num2str(U),'_V1=',num2str(V1),'_V2=',num2str(V2),'_V3=',num2str(V3),'.mat'];
% save(str,'P','C','D','vmaxflow','qvecs');
% 
% str=['Kagome_spinless_Tflow_',num2str(Nf),'FFs_qmesh=',num2str(Lq),'_kmesh=',num2str(Lk),'_mu=',num2str(mu),'_U=',num2str(U),'_V1=',num2str(V1),'_V2=',num2str(V2),'_V3=',num2str(V3),'_orderflow.mat'];
% if (orderflow_track)
%     KevalGflow=KevalGflow(1:iter,:);
%     KevecGflow=KevecGflow(:,:,1:iter);
%     KevalMflow=KevalMflow(1:iter,:);
%     KevecMflow=KevecMflow(:,:,1:iter);
%     save(str,'KevalGflow','KevecGflow','KevalMflow','KevecMflow');
% end

% str=['Kagome_spinless_Tflow_',num2str(Nf),'FFs_qmesh=',num2str(Lq),'_kmesh=',num2str(Lk),'_Vmax=',num2str(divergence_criterion),'_mu=',num2str(mu),'_U=',num2str(U),'_V1=',num2str(V1),'_V2=',num2str(V2),'_V3=',num2str(V3),'.mat'];
% if (ledingorderflow_track==1)
%     KevalGflow=KevalGflow(1:iter,:);
%     KevecGflow=KevecGflow(:,:,1:iter);
%     KevalMflow=KevalMflow(1:iter,:);
%     KevecMflow=KevecMflow(:,:,1:iter);
%     PevalGflow=PevalGflow(1:iter,:);
%     PevecGflow=PevecGflow(:,:,1:iter);
%     PevalMflow=PevalMflow(1:iter,:);
%     PevecMflow=PevecMflow(:,:,1:iter);
...
% str=['Kagome_spinless_Tflow_',num2str(Nf),'FFs_qmesh=',num2str(Lq),'_kmesh=',num2str(Lk),'_Vmax=',num2str(divergence_criterion),'_mu=',num2str(mu),'_U=',num2str(U),'_V1=',num2str(V1),'_V2=',num2str(V2),'_V3=',num2str(V3),'.mat'];
str=['Kagome_spinless_Tflow_',num2str(Nf),'FFs_qmesh=',num2str(Lq),'_kmesh=',num2str(Lk),'_Vmax=',num2str(vstop),'_mu=',num2str(mu),'_U=',num2str(U),'_V1=',num2str(V1),'_V2=',num2str(V2),'_V3=',num2str(V3),'.mat'];
